...
class Pet(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    species = db.Column(db.String(50))  # cachorro, gato, etc
    breed = db.Column(db.String(100))
    photo_url = db.Column(db.String(500))
    device_id = db.Column(db.String(100), unique=True)  # ID único do ESP32
    api_key = db.Column(db.String(100), unique=True)  # Chave de API para o dispositivo
...
class Location(db.Model):
    ...
    altitude = db.Column(db.Float)  # Opcional
    speed = db.Column(db.Float)  # Velocidade em km/h
    satellites = db.Column(db.Integer)  # Número de satélites
    hdop = db.Column(db.Float)  # Precisão horizontal
...
class GeofenceZone(db.Model):
    ...
    radius_meters = db.Column(db.Float, nullable=False)  # Raio em metros
...
class Alert(db.Model):
    """Modelo de alertas (pet saiu da cerca, bateria baixa, etc)"""
    __tablename__ = 'alerts'

    id = db.Column(db.Integer, primary_key=True)
    pet_id = db.Column(db.Integer, db.ForeignKey('pets.id'), nullable=False)
    alert_type = db.Column(db.String(50), nullable=False)  # geofence, battery, offline
